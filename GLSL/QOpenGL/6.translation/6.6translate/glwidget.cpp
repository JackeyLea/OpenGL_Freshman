...
void GLWidget::setTriangleChecked(bool check)
{
    rchecked = check;
}

void GLWidget::setQuadChecked(bool check)
{
    qchecked = check;
}

void GLWidget::updown(float y)
{
    if(rchecked){
        ry+=y;
    }
...
void GLWidget::leftright(float x)
{
    if(rchecked){
        rx+=x;
    }
...
void GLWidget::remoteclose(float z)
{
    if(rchecked){
        rz+=z;
    }
...
void GLWidget::paintGL()
{
    ...
    m_modelView.setToIdentity();
    if(rchecked){
        m_modelView.translate(-1.5f+rx, 0.0f+ry, -6.0f+rz);
    }
    ...
    m_modelView.setToIdentity();
    if(qchecked){
        m_modelView.translate(1.5f+qx, 0.0f+qy, -6.0f+qz);
    }
    program->setUniformValue("mvpMatrix", m_projection * m_modelView);
    glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[2]);
    program->enableAttributeArray(0);
